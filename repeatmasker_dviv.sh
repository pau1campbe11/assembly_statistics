#!/bin/bash -l

################ SLURM SETTINGS ################
#SBATCH --account=project0005              # Replace with your account/project name
#SBATCH --job-name=repeatmask_dviv      # Descriptive job name
#SBATCH --output=%x-%J.out                 # Output file with job name and ID
#SBATCH --error=%x-%J.err                  # Error file
#SBATCH --time=0-04:00:00                  # Max wall time
#SBATCH --mem=20G                          # Total memory per node
#SBATCH --ntasks=1                         # One SLURM task
#SBATCH --cpus-per-task=12                 # Use 16 cores for RepeatMasker
#SBATCH --ntasks-per-node=1
#SBATCH --mail-user=p.campbell.5@research.gla.ac.uk
#SBATCH --mail-type=ALL

################ LOAD MODULES / ENVIRONMENT ################
module load apps/miniforge
conda activate repeatmasker_4.1.8

################# INPUTS #################
GENOME=/users/2320707c/project0005/for_paul_oster/ref_genomes/GCA_964187775.1_ngDicVivi2.1_genomic.fna 
REPEAT_LIBRARY=/users/2320707c/project0005/for_paul_oster/ref_genomes/repeatm/rmod_dviv/d.viviparus_db-families.fa
OUTDIR=/users/2320707c/project0005/for_paul_oster/ref_genomes/repeatm/rmask_dviv

mkdir -p ${OUTDIR}

################ RUN REPEATMASKER ################

RepeatMasker \
  -pa 16 \
  -lib ${REPEAT_LIBRARY} \
  -gff \
  -dir ${OUTDIR} \
  -a \
  -s \
  -poly \
  -small \
  -html \
  -e ncbi \
  ${GENOME}


  
